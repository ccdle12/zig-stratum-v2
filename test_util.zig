...
pub fn serdeTestNoAlloc(comptime T: type, before: T, comptime size: comptime_int, expected: [size]u8) !T {
    const buf = try serdeTest(T, before, size, expected);
    defer buf.deinit();
    return try T.read(io.fixedBufferStream(buf.items).reader());
}

pub fn serdeTestAlloc(comptime T: type, gpa: *mem.Allocator, before: T, comptime size: comptime_int, expected: [size]u8) !T {
    const buf = try serdeTest(T, before, size, expected);
    defer buf.deinit();
...
pub fn frameTestNoAlloc(comptime T: type, before: T, comptime size: comptime_int, expected: [size]u8) !T {
    const buf = try frameTest(T, before, size, expected);
    defer buf.deinit();
    return try T.unframe(io.fixedBufferStream(buf.items).reader());
}

pub fn frameTestAlloc(comptime T: type, gpa: *mem.Allocator, before: T, comptime size: comptime_int, expected: [size]u8) !T {
    const buf = try frameTest(T, before, size, expected);
    defer buf.deinit();
    return try T.unframe(gpa, io.fixedBufferStream(buf.items).reader());
}

fn frameTest(comptime T: type, before: T, comptime size: comptime_int, expected: [size]u8) !std.ArrayList(u8) {
    var buf = std.ArrayList(u8).init(testing.allocator);
